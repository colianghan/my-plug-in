		...
		if(!$.browser.safari&&typeof window.console!=="undefined"&&typeof window.console.log==="function"){
			$.log=function(){
				...
				for(var i=0;i<arguments.length;i++){
					args.push(arguments[i])}window.console.log.apply(window.console,args)
				};
				$.console={
					log:$.log,debug:window.console.debug||$.log,warn:window.console.warn||$.log,error:window.console.error||$.log,trace:window.console.trace||$.log
				}
			}
			else{
				$.log=function(){};
			...
			$.params_to_json=$.params_to_json||function(params){params=String(params);
				...
				for(var i=0,n=params.length;i<n;++i){
					...
					if(keys.length===1){
						...
						for(ii in keys){
							key=keys[ii];
							path+="."+key;eval("json"+path+" = json"+path+" || {}")
						}
			...
			$.fn.lightbox=function(options){
				...
				if($.Lightbox.ie6&&!$.Lightbox.ie6_support){
					return this}options=$.extend({start:false,events:true},options);
					var group=$(this);
					if(options.events){
					...
					$.extend($.LightboxClass.prototype,{images:{list:[],image:false,prev:function(image){
											...
											return this.list.length},empty:function(){
												return this.size()===0},clear:function(){
													this.list=[];this.image=false},active:function(image){
														...
														return true},create:function(obj){
															var image={src:"",title:"Untitled",description:"",name:"",index:-1,color:null,width:null,height:null,image:true};
															if(obj.image){
																image.src=obj.src||image.src;image.title=obj.title||image.title;
																...
																var prepend=function(item,value){
																	...
																	$("body").append('<div id="lightbox-overlay"><div id="lightbox-overlay-text">'+
																		...
																		(this.show_helper_text?'<p><span id="lightbox-overlay-text-close">'+
																			...
																			this.text.help.interact+"</span></p>":"")+
																		...
																		'" /></a></div></div></div></div>');
this.resizeBoxes();this.repositionBoxes();
...
if(this.show_linkback){
	...
	$("#lightbox-overlay-text-interact").fadeIn()},function(){
		$("#lightbox-overlay-text-interact").fadeOut()});
$("#lightbox-caption-title").click(function(){
	window.open($(this).attr("href"));
	return false
});
$("#lightbox-overlay, #lightbox, #lightbox-loading-link, #lightbox-btnClose").unbind().click(
	function(){$.Lightbox.finish();return false});
if(this.auto_relify){this.relify()}return true},relify:function(){
	var groups={};var groups_n=0;
	...
	if(type!=="transition"){
		...
		if(type==="transition"){
			if(dWidth===0&&dHeight===0){
				...
				$("#lightbox-imageBox").animate({
					width:nWidth,height:nHeight},this.speed,function(){
		...
		this.repositionBoxes({nHeight:nHeight,speed:this.speed});
		return true},repositioning:false,reposition_failsafe:false,repositionBoxes:function(options){
			...
			if(options.speed){$("#lightbox").animate(css,"slow",function(){
		...
		return true
	},visible:false,showImage:function(image,step){
		image=this.images.get(image);
		if(!image){
			return image
		}
		step=step||1;var skipped_step_1=step>1&&this.images.active().src!==image.src;
		var skipped_step_2=step>2&&$("#lightbox-image").attr("src")!==image.src;if(skipped_step_1||skipped_step_2){
		...
		switch(step)
		{
			...
			if(!this.images.active(image)){
				...
				preloader.onload=function(){
					...
					preloader.onload=null;preloader=null
				};
			...
			if(typeof this.padding==="undefined"||this.padding===null||isNaN(this.padding)){
				this.padding=parseInt($("#lightbox-imageContainer").css("padding-left"),10)||parseInt($("#lightbox-imageContainer").css("padding"),10)||0}if(this.colorBlend){
				...
				if(this.download_link){$title.attr("href",this.download_link?image.src:"")}delete $title;
				...
				if(key===this.keys.prev||keycode===37){
					...
					var arrayPageScroll={
						...
						return arrayPageScroll},pause:function(ms){
							var date=new Date();
							var curDate=null;
							do{
								curDate=new Date()
